	...
	class acf_field_relationship_multisite extends acf_field {
		...
		*/
		...
		function ajax_query() {
			...
			$args = apply_filters( 'acf/fields/relationship/query/name=' .
			                       $field['name'], $args, $field, $options['post_id'] );
			$args = apply_filters( 'acf/fields/relationship/query/key=' .
			                       $field['key'], $args, $field, $options['post_id'] );
		...
		*/
		...
		function get_post_title( $post, $field, $post_id = 0 ) {
			...
			$title = apply_filters( 'acf/fields/relationship/result/name=' .
			                        $field['_name'], $title, $post, $field, $post_id );
			$title = apply_filters( 'acf/fields/relationship/result/key=' .
			                        $field['key'], $title, $post, $field, $post_id );
		...
		*/
		...
		function render_field( $field ) {
			...
			echo '<p>' . __( 'You are looking on posts from site', 'acf-relationship-multisite' ) . ' <strong>' .
			     get_bloginfo( 'name' ) . '</strong></p>';
			restore_current_blog();
							...
							<?php if ( ! empty( $field['value'] ) ):
								...
								if ( ! empty( $posts ) ):
									...
									<?php echo $this->get_post_title( $post, $field ); ?>
                                            <a href="#" class="acf-icon small dark" data-name="remove_item"><i
                                                        class="acf-sprite-remove"></i></a>
								</span>
                                        </li><?php
...
// initialize
	new acf_field_relationship_multisite( $this->settings );
// class_exists check
endif;
